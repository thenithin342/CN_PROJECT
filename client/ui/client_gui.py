...
class ClientMainWindow(QMainWindow):
    ...
    def initialize_client_modules(self):
        ...
        try:
            ...
            # Set frame callback for screen viewer to integrate with video grid
            self.screen_viewer.set_frame_callback(self._on_frame_received)
    ...
    def _watch_screen_share(self, uid: int, viewer_port: int, username: str):
        ...
        try:
            ...
            from client.screen.screen_viewer import SCREEN_SHARE_AVAILABLE

            if not SCREEN_SHARE_AVAILABLE:
                self.chat_widget.add_message("System", "Screen sharing not available", is_system=True)
                return

            # Add screen share feed to video grid
            if uid not in self.video_grid.video_frames:
                self.video_grid.add_video_feed(uid, f"{username} (Screen)")
                print(f"[GUI] Added screen share feed for uid={uid}, username={username}")

            # Start receiving frames using network thread's event loop
            if self.network_thread and self.network_thread.loop:
                asyncio.run_coroutine_threadsafe(
                    self.screen_viewer.view_presentation(viewer_port, username, uid),
                    self.network_thread.loop
                )
            else:
                self.chat_widget.add_message("System", "Network thread not available for screen viewing", is_system=True)
                return
    ...
    def _on_screen_view_complete(self, success: bool, error: str, result, username: str, sender=None):
        ...
        if success:
            ...
            # Remove screen share feed from video grid on failure
            if self.screen_viewer and self.screen_viewer.current_presentation:
                uid = self.screen_viewer.current_presentation.get('uid')
                if uid and uid in self.video_grid.video_frames:
                    self.video_grid.remove_video_feed(uid)
    ...
    def handle_present_stop_broadcast(self, message: dict):
        ...
        # Remove screen share feed from video grid
        if uid in self.video_grid.video_frames:
            self.video_grid.remove_video_feed(uid)
            print(f"[GUI] Removed screen share feed for uid={uid}")
